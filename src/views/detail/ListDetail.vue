  ...
  <div>
      ...
      <el-col :span="18" :offset="0">
        <el-row :gutter="0">
          <el-col :span="1" :offset="0">
            <div class="list-tag">歌单</div></el-col
          >
          <el-col :span="22" :offset="0"
            ><span class="list-title">{{ playlist.name }}</span></el-col
          >
        </el-row>
        <el-row>
          <el-col :span="4">
        ...
        <div class="list-des">
          标签：<span v-for="item in playlist.tags" :key="item.index">
            {{ item }}
          </span>
        </div>
        <div class="list-des">
          歌曲：{{ playlist.trackCount }} &ensp;播放：{{
            getPlayCount(playlist.playCount)
          }}
        </div>
        <div class="list-des">简介：{{ playlist.description }}</div>
      </el-col>
    </el-row>
    <el-tabs v-model="activeName" class="list-tabs-control" @tab-click="handleClick">
      <el-tab-pane label="歌曲列表" name="songs">
        ...
        <more-song-list :id="this.id" ref="morelist"></more-song-list>
      </el-tab-pane>
      <el-tab-pane
        :label="'评论(' + playlist.commentCount + ')'"
        name="comments"
        :labelContent="commentTab"
        >
...
<script>
import { getListDetail } from "network/songList.js";
// import {Bus} from 'utils/eventBus.js';
import MoreSongList from "./children/MoreSongList.vue";
...
export default {
  ...
  created() {
    ...
    getListDetail(this.id).then((res) => {
      ...
      this.songId = res.data.privileges[0].id
    });
  },
  methods: {
    getPlayCount(count) {
    ...
    sendComment() {
      sendComment(1, 2, this.id, this.textarea).then((res) => {
        console.log(res);
        this.textarea = "";
        setTimeout(() => {
